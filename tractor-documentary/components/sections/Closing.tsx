  ...
  return (
    <section id="prototype" className="section" style={{ background: "var(--bg-elev)" }}>
      <div className="wrap">
        <SectionHeader
          index="17"
          kicker="Prototype & Validation"
          title="27 screens. 10 chapters. One testable story."
          lead="The full design space was reduced to the 27 essential screens that carry the testable story, organised into 10 chapters. A prototype isn't a museum of every screen drawn — it's the shortest path that still lets a real person complete a real task."
        />
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          {["Try: Decode a fault alert", "Try: Book a dealer service", "Try: Order an oil filter", "Try: Complete the RDI checklist"].map((t) => (
            <span
              key={t}
        ...
        <p className="mt-4 text-xs" style={{ color: "var(--ink-faint)" }}>
          Point the launch target at your Figma prototype URL and embed a walkthrough video here.
        </p>
      </div>
    </section>
  );
    ...
    <section id="tech" className="section">
      <div className="wrap">
        <SectionHeader
          ...
          title="Nine participants. Six tasks. Three real fixes."
          lead={tech.lead}
        />
        <div className="mt-14 grid gap-12 lg:grid-cols-2">
          <div>
            <Reveal><p className="eyebrow mb-5">The test plan</p></Reveal>
          ...
          <Reveal dir="left">
            <div className="card h-full p-8">
              <div className="eyebrow mb-5">What testing forced</div>
              <ul className="space-y-5">
                {tech.future.map((f) => (
                  <li key={f} className="flex gap-3 text-sm leading-relaxed" style={{ color: "var(--ink-dim)" }}>
              ...
              <p className="mt-8 text-xs leading-relaxed" style={{ color: "var(--ink-faint)" }}>
                Small changes, but exactly the plain-language and accessibility discipline the whole product is built on.
              </p>
            </div>
          </Reveal>
        </div>
    ...
    <section id="impact" className="section" style={{ background: "var(--bg-elev)" }}>
      <div className="wrap">
        <SectionHeader
          index="19"
          kicker="Impact"
          title="What changed. How we measured it."
          lead={impact.note}
        />
        ...
        <Stagger className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4" gap={0.1}>
          {impact.kpis.map((k) => (
            <motion.div key={k.label} variants={staggerItem} className="card p-7">
              <div
                className="text-5xl font-light leading-none"
                style={{ fontFamily: "var(--font-display)", color: k.tone === "green" ? "var(--green)" : "var(--amber)" }}
              >
                <Counter to={k.value} />
                <span>{k.suffix}</span>
              </div>
              <div className="mt-4 text-sm font-medium" style={{ color: "var(--ink)" }}>{k.label}</div>
              <div className="mt-1 text-xs" style={{ color: "var(--ink-faint)" }}>{k.target} · met</div>
            </motion.div>
          ))}
        </Stagger>
    ...
    <section id="reflection" className="section">
      <div className="wrap">
        <SectionHeader
          index="20"
          kicker="Reflection"
          title="What we learned. What we'd do differently."
        />
    ...
    <section id="takeaways" className="section">
      ...
      >
        <p className="text-sm" style={{ color: "var(--ink-dim)" }}>
          Tractor Connect 2.0 · Connected Tractor Companion App · UX/UI Case Study
        </p>
        <p className="mt-2 text-xs" style={{ color: "var(--ink-faint)" }}>
          Dayala Sankaran · Lead UX Designer · Built with Next.js, React, Tailwind &amp; Framer Motion
        </p>
      </footer>
    </section>
  );
